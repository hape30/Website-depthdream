...
def start_conversation(user_id, username=None):
    logging.info(f"Starting conversation with user_id: {user_id}, username: {username}")
    ...
    logging.info(f"Conversation started for user_id: {user_id}")

def is_conversation_active(user_id):
    active = active_sessions.get(user_id, False)
    logging.info(f"Checking if conversation is active for user_id: {user_id} - {active}")
    return active

def get_active_user_id_for_tarologist():
    conversation = session_tarologist.query(Conversation).filter_by(active=True).first()
    logging.info(f"Getting active user id for tarologist: {conversation.user_id if conversation else 'None'}")
    return conversation.user_id if conversation else None

def save_message(user_id, sender_id, message_text):
    logging.info(f"Saving message from sender_id: {sender_id} to user_id: {user_id} - {message_text}")
...
def save_bot_message(message_text):
    logging.info(f"Saving bot message: {message_text}")
    ...
    session_bot.commit()
    logging.info("Bot message saved")
